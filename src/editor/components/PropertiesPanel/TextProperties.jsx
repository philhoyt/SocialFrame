		...
		<div className="socialframe-props">
			<Accordion title={ __( 'Position', 'socialframe' ) }>
				<div className="socialframe-num-grid">
					...
					<NumField
						label="W"
						value={ Math.round( props.width ?? 0 ) }
						onChange={ ( v ) => update( { width: v }, 'Width' ) }
					/>
					<NumField
						label="H"
						value={ Math.round( props.height ?? 0 ) }
						readOnly
					/>
					<NumField
						label="°"
			...
			<Accordion title={ __( 'Typography', 'socialframe' ) }>
				{ fontOptions.length > 0 && (
					<SelectControl
						...
						options={ fontOptions }
						onChange={ ( v ) =>
							update( { fontFamily: v }, 'Font family' )
						}
					/>
				) }

				<div
					className="socialframe-num-grid"
				...
				>
					...
					<NumField
						...
						max={ 4 }
						onChange={ ( v ) =>
							update( { lineHeight: v }, 'Line height' )
						}
					/>
					<NumField
						label="LS"
						...
						max={ 800 }
						onChange={ ( v ) =>
							update( { charSpacing: v }, 'Letter spacing' )
						}
					/>
				</div>

				<div className="socialframe-props__subsection-label">
					{ __( 'Style', 'socialframe' ) }
				</div>
				<div
				...
				<div className="socialframe-props__button-group">
					{ [
						...
						{
							value: 'center',
							label: __( 'Align center', 'socialframe' ),
							icon: (
								<svg
									width="16"
									height="16"
			...
			<Accordion title={ __( 'Fill', 'socialframe' ) }>
				<ColorRow
					...
					onChange={ ( hex ) =>
						update( { fill: hex }, 'Text color' )
					}
				/>
			</Accordion>

			<Accordion
				title={ __( 'Background', 'socialframe' ) }
				defaultOpen={ false }
			>
				<ColorRow
					value={ props.backgroundColor || null }
					onChange={ ( hex ) =>
						update( { backgroundColor: hex }, 'Text background' )
					}
					showNone
					noneValue=""
