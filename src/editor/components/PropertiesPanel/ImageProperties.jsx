		...
		<div className="socialframe-props">
			...
			<Accordion title={ __( 'Position', 'socialframe' ) }>
				<div className="socialframe-num-grid">
					...
					<LinkedNumFields
						stateKey="wh"
						aLabel="W" aValue={ props.width  ?? 0 } aMin={ 1 } onChangeA={ ( v ) => update( { scaleX: v / ( props.naturalWidth  ?? 100 ) }, 'Width' ) }
						bLabel="H" bValue={ props.height ?? 0 } bMin={ 1 } onChangeB={ ( v ) => update( { scaleY: v / ( props.naturalHeight ?? 100 ) }, 'Height' ) }
						onChangeBoth={ ( w, h ) => update( { scaleX: w / ( props.naturalWidth ?? 100 ), scaleY: h / ( props.naturalHeight ?? 100 ) }, 'Size' ) }
					/>
