...
const PANELS = [
	...
	{ id: 'text', icon: <IconText />, label: __( 'Text', 'socialframe' ) },
	{
		id: 'elements',
		icon: <IconElements />,
		label: __( 'Elements', 'socialframe' ),
	},
	{ id: 'media', icon: <IconMedia />, label: __( 'Media', 'socialframe' ) },
	{
		id: 'posts',
		icon: <IconPosts />,
		label: __( 'Import Post', 'socialframe' ),
	},
	{
		id: 'layers',
		icon: <IconLayers />,
		label: __( 'Layers', 'socialframe' ),
	},
];
...
export function IconNav() {
	...
	return (
		<nav
			className="socialframe-editor__icon-nav"
			aria-label={ __( 'Editor panels', 'socialframe' ) }
		>
			{ PANELS.map( ( { id, icon, label } ) => (
				<Tooltip key={ id } text={ label } placement="right">
					<Button
						className={ `socialframe-icon-nav__btn${
