 ...
 * @returns {Promise<{attachmentId, url, libraryUrl}>}
 */
export async function exportDesign( toDataURL, designId, apiFetch ) {
	const imageData = toDataURL();
	if ( ! imageData ) throw new Error( 'Canvas produced no image data.' );
 ...
 * @returns {Promise<Object>} New template post data.
 */
export async function saveAsTemplate( { title, format, fabricJson, apiFetch } ) {
	return apiFetch( {
