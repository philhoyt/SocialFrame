...
#
# This is the canonical WordPress core script (develop.svn.wordpress.org), kept
# in-repo so contributors can bootstrap the integration test suite without Docker.
...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
		# version x.x.0 means the first release of the major version, so strip off the .0 and download the x.x version.
